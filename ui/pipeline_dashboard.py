                    ...
                    with st.expander(f"{appr_icon}{val_icon} {card.name}", expanded=not is_approved):
                        ...
                        if vr:
                            ...
                            if has_issues:
                                ...
                                # Build AC suggestion comment from gaps + suggestions
                                _has_ac_content = vr.ac_gaps or vr.suggestions
                                if _has_ac_content:
                                    _push_key = f"ac_pushed_{card.id}"
                                    if st.session_state.get(_push_key):
                                        st.success("✅ Suggested AC pushed to Trello comment!")
                                        if st.button("📤 Push again", key=f"ac_push_again_{card.id}"):
                                            st.session_state[_push_key] = False
                                            st.rerun()
                                    else:
                                        if st.button(
                                            "📤 Push Suggested Acceptance Criteria",
                                            key=f"push_ac_{card.id}",
                                            type="primary",
                                            use_container_width=True,
                                        ):
                                            # Format AC suggestion as a Trello comment
                                            _lines = ["🤖 **Suggested Acceptance Criteria (Domain Expert)**", ""]
                                            if vr.ac_gaps:
                                                _lines.append("**📋 Missing AC Scenarios:**")
                                                for _gap in vr.ac_gaps:
                                                    _lines.append(f"- {_gap}")
                                                _lines.append("")
                                            if vr.suggestions:
                                                _lines.append("**💡 Suggestions:**")
                                                for _sug in vr.suggestions:
                                                    _lines.append(f"- {_sug}")
                                                _lines.append("")
                                            if vr.requirement_gaps:
                                                _lines.append("**⚠️ Requirement Gaps to address:**")
                                                for _rg in vr.requirement_gaps:
                                                    _lines.append(f"- {_rg}")
                                                _lines.append("")
                                            _lines.append("_Generated by TrackingApp Domain Expert — please review before finalising AC._")
                                            _comment_text = "\n".join(_lines)
                                            with st.spinner("Pushing to Trello…"):
                                                TrelloClient().add_comment(card.id, _comment_text)
                                            st.session_state[_push_key] = True
                                            st.rerun()
